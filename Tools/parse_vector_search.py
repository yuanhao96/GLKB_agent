...
class DocumentModel(BaseModel):
    ...
    def extract_abstract(self) -> str:
        # Extract the title from page_content
        match = re.search(r'abstract: (.+)', self.page_content)
        if match:
            return match.group(1)
        return ""
...
def create_context(state: GraphState):
    ...
    extracted_data = [{"title": doc.extract_title(), "abstract":doc.extract_abstract(), "article_id": doc.metadata.article_id} for doc in documents]
