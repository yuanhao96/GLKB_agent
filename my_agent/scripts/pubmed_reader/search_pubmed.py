...
from .fetch_article import batch_fetch
...
def search_pubmed(
    ...
    # Check cache
    cache_key = f"{query}:{max_results}:{min_date}:{max_date}:{sort}:{include_abstracts}"
    if use_cache:
    ...
    # Fetch abstracts and merge into summaries
    if include_abstracts and pmids and result.get("articles"):
        try:
            batch_result = batch_fetch(pmids)
            if batch_result.get("success"):
                abstract_map = {}
                for bid, article_data in batch_result.get("articles", {}).items():
                    if article_data.get("abstract"):
                        abstract_map[bid] = article_data["abstract"]
                for article in result["articles"]:
                    aid = article.get("pmid", "")
                    if aid in abstract_map:
                        article["abstract"] = abstract_map[aid]
        except Exception as e:
            logger.warning(f"Failed to fetch abstracts for search results: {e}")
