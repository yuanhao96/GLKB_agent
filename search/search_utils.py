...
from edges import SemanticEdge, get_semantic_edge_from_record
from graph_queries import (
    get_nodes_query,
    get_relationships_query,
    get_vector_cosine_func_query,
    get_nodes_similarity_query,
)
from helpers import (
    RUNTIME_QUERY,
    lucene_sanitize,
...
from models.edges.edge_db_queries import SEMANTIC_EDGE_RETURN
from models.nodes.node_db_queries import VOCABULARY_NODE_RETURN, SENTENCE_NODE_RETURN, ARTICLE_NODE_RETURN
from nodes import (
    ArticleNode,
    SentenceNode,
    VocabularyNode,
    get_article_node_from_record,
    get_sentence_node_from_record,
    get_vocabulary_node_from_record,
)
from search.search_filters import (
    SearchFilters,
...
async def get_articles_by_vocabulary(
    driver: GraphDriver,
    ...
    for edge in edges:
        article_ids.extend([f'pmid{pmid}' for pmid in edge.pubmedids])

    articles = await ArticleNode.get_by_ids(driver, article_ids[:limit])
    articles += await ArticleNode.get_by_vocabulary_ids(driver, [node.id for node in nodes], limit=limit)

    return articles

async def get_sentences_by_vocabulary(
    driver: GraphDriver,
    nodes: list[VocabularyNode] = [],
    edges: list[SemanticEdge] = [],
    limit: int = RELEVANT_SCHEMA_LIMIT,
    ...
    for edge in edges:
        sentence_ids.extend(edge.evidence)

    sentences = await SentenceNode.get_by_ids(driver, sentence_ids[:limit])
    sentences += await SentenceNode.get_by_vocabulary_ids(driver, [node.id for node in nodes], limit=limit)

    return sentences


async def get_mentioned_nodes(
    driver: GraphDriver, sentences: list[SentenceNode]
) -> list[VocabularyNode]:
    sentence_ids = [sentence.id for sentence in sentences]

    records, _, _ = await driver.execute_query(
        """
        MATCH (s:Sentence)-->(:GenomicMention)-->(n:Vocabulary)
        WHERE s.id IN $uuids
        RETURN DISTINCT
        """
...
#     driver: GraphDriver, nodes: list[VocabularyNode]
# ) -> list[CommunityNode]:
...
async def edge_fulltext_search(
    ...
    query = (
        get_relationships_query('Semantic_rels')
        + """
        YIELD relationship AS rel, score
        MATCH (n:Vocabulary)-[e:Cooccur]->(m:Vocabulary) WHERE elementId(e) = elementId(rel)"""
        + filter_query
        + """
        WITH e, score, n, m
        RETURN
    ...
    edges = [get_semantic_edge_from_record(record) for record in records]
...
async def edge_bfs_search(
    ...
    query = (
        f"""
            ...
            MATCH path = (origin:Vocabulary {{id: origin_uuid}})-[:Cooccur*1..{bfs_max_depth}]->(:Vocabulary)
            UNWIND relationships(path) AS rel
            MATCH (n:Vocabulary)-[e:Cooccur]-(m:Vocabulary)
            WHERE elementId(e) = elementId(rel)
        """
        + filter_query
        + """
        RETURN DISTINCT
...
async def node_fulltext_search(
    ...
    query = (
        get_nodes_query(driver.provider, 'vocabulary_Names', '$query')
        + """
        YIELD node AS n, score
        """
        + filter_query
        + """
        WHERE apoc.node.degree(n) > 0
        """
        + """
        WITH n, score
        ORDER BY score DESC
...
async def node_similarity_search(
    ...
    #     query_params['group_ids'] = group_ids
    group_filter_query = 'WHERE n.embedding IS NOT NULL'
    ...
    query = (
        ...
        + """
        WITH n, """
        + get_vector_cosine_func_query('n.embedding', '$search_vector', driver.provider)
        + """ AS score
        WHERE score > $min_score
        RETURN
        """
...
async def node_bfs_search(
    ...
    query = (
        f"""
            UNWIND $bfs_origin_node_uuids AS origin_uuid
            MATCH (origin:Vocabulary {{id: origin_uuid}})-[:Cooccur*1..{bfs_max_depth}]->(n:Vocabulary)
        """
        + filter_query
        + """
        RETURN
...
async def article_fulltext_search(
    driver: GraphDriver,
    query: str,
    search_filter: SearchFilters,
    group_ids: list[str] | None = None,
    ...
    query = (
        get_nodes_query(driver.provider, 'article_Title', '$query')
        + """
        YIELD node AS n, score
        """
        + filter_query
    ...
    nodes = [get_article_node_from_record(record) for record in records]

    return nodes

async def sentence_fulltext_search(
    driver: GraphDriver,
    query: str,
    search_filter: SearchFilters,
    group_ids: list[str] | None = None,
    ...
    query = (
        get_nodes_query(driver.provider, 'Sentences', '$query')
        + """
        YIELD node AS n, score
        """
        + filter_query
...
async def article_similarity_search(
    driver: GraphDriver,
    search_vector: list[float],
    group_ids: list[str] | None = None,
    limit=RELEVANT_SCHEMA_LIMIT,
    ...
    #     WITH n,
    #     """
    #     + get_vector_cosine_func_query('n.openai_embedding', '$search_vector', driver.provider)
    #     + """ AS score
    #     WHERE score > $min_score
    #     RETURN
    #     """
    ...
    # )
    query = (
    get_nodes_similarity_query('article_embeddings', search_vector, limit) + """
    YIELD node AS n, score
    WHERE score > $min_score
    RETURN
    """ 
...
async def hybrid_node_search(
    ...
    start = time()
    results: list[list[VocabularyNode]] = list(
    ...
    node_uuid_map: dict[str, VocabularyNode] = {
        node.id: node for result in results for node in result
    }
    result_uuids = [[node.id for node in result] for result in results]

    ranked_uuids, _ = rrf(result_uuids)

    relevant_nodes: list[VocabularyNode] = [node_uuid_map[uuid] for uuid in ranked_uuids]
...
async def get_relevant_nodes(
    ...
    query = (
        ...
        + """
        WITH node, n, """
        + get_vector_cosine_func_query('n.embedding', 'node.embedding', driver.provider)
        + """ AS score
        WHERE score > $min_score
    ...
    relevant_nodes_dict: dict[str, list[VocabularyNode]] = {
        result['search_node_uuid']: [
            get_vocabulary_node_from_record(record) for record in result['matches']
        ]
...
#             get_semantic_edge_from_record(record) for record in result['matches']
#         ]
#         for result in results
#     }
...
async def node_distance_reranker(
    ...
    results, header, _ = await driver.execute_query(
        ...
        MATCH (center:Vocabulary {id: $center_uuid})--(n:Vocabulary {id: node_uuid})
...
async def article_vocabulary_reranker(
    ...
    results, _, _ = await driver.execute_query(
        """
        UNWIND $node_uuids AS node_uuid
        MATCH (article:Article)-[r:ContainTerm]->(n:Vocabulary {id: node_uuid})
        RETURN count(*) AS score, n.id AS uuid
        """,
...
async def get_embeddings_for_vocabulary(
    driver: GraphDriver, nodes: list[VocabularyNode]
) -> dict[str, list[float]]:
    results, _, _ = await driver.execute_query(
        """
        MATCH (n:Vocabulary)
        WHERE n.id IN $node_uuids
        RETURN DISTINCT
            n.id AS uuid,
            n.embedding AS embedding
        """,
        node_uuids=[node.id for node in nodes],
        routing_='r',
    )
    ...
    async def get_embeddings_for_articles(
        driver: GraphDriver, articles: list[ArticleNode]
    ) -> dict[str, list[float]]:
        results, _, _ = await driver.execute_query(
            """
            MATCH (a:Article)
            WHERE a.id IN $article_uuids
            RETURN DISTINCT
                a.id AS uuid,
                a.openai_embedding AS embedding
            """,
            article_uuids=[article.id for article in articles],
            routing_='r',
        )
...
#     driver: GraphDriver, edges: list[SemanticEdge]
# ) -> dict[str, list[float]]:
#     results, _, _ = await driver.execute_query(
#         """
#         MATCH (n:Vocabulary)-[e:Cooccur]-(m:Vocabulary)
#         WHERE elementId(e) IN $edge_uuids
#         RETURN DISTINCT
#             elementId(e) AS uuid,
